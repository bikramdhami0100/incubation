...
export default function CommitteesPage() {
  ...
  const { data: committeesData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/committee?search=${searchTerm}&page=${currentPage}`
      );
  ...
  const filteredAndSortedCommittees =
    committeesData?.data
      ?.filter(
        (committee) =>
  ...
  // Committee details modal after click on view Details
                    ...
                    <div className="flex items-center space-x-4 mt-2 text-blue-100">
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-sm">
                          Updated{" "}
                          {getRelativeTime(selectedCommittee.updated_at)}
                        </span>
                      </div>
                      <div className="flex items-center space-x-1">
                ...
                {members.map((member, index) => (
                  ...
                  >
                    <div className="flex flex-col items-center text-center">
                      <div onClick={()=>{
                         if(member.photo && typeof window !== "undefined"){
                          window.open(`${process.env.NEXT_PUBLIC_IMAGE_BASE_URL}/committees/${member.photo}`,"_blank"); 
                         }
                      }} className="w-24 cursor-pointer h-24 mb-4 rounded-full overflow-hidden">
  ...
  // generate the random gradient background color
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="text-center">
            ...
            <p className="text-xl text-blue-100 max-w-3xl mx-auto mb-12 leading-relaxed">
              Discover the dedicated teams driving innovation, growth, and
                  ...
                  {searchTerm ? (
                    <span>
                      ...
                      <span className="font-bold text-blue-600">
                        {filteredAndSortedCommittees.length}
                      </span>{" "}
                      committees matching
